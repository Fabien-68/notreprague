...
const ItineraryPanel: React.FC<ItineraryPanelProps> = ({
  ...
  const generateGoogleMapsUrl = () => {
    ...
    const waypoints = itinerary
      .map(place => `${place.coordinates.lat},${place.coordinates.lng}`)
      .join('/');
    ...
    <div className="sticky top-8 p-6 bg-black bg-opacity-50 border border-gray-700 rounded-lg backdrop-blur-sm">
      ...
      {itinerary.length === 0 ? (
        <p className="text-gray-400">
          Ajoutez des lieux pour commencer à planifier votre parcours.
        </p>
      ) : (
            ...
            {itinerary.map((place, index) => {
              ...
              return (
                <div key={place.id} className="mb-2">
                  ...
                  <div
                    className={`flex items-center justify-between bg-white/10 p-2 rounded-md ${
                  ...
                  >
                    <div className="flex items-center min-w-0">
                      ...
                      <p
                        className={`flex-grow text-gray-200 truncate ${index === 0 ? 'font-bold' : ''}`}
                        title={place.name}
                      >
                        {place.name}
                      </p>
